...
<#
.SYNOPSIS
    Deploy wrapper for a single manual-copy system:
    gate -> stop -> backup -> copy -> restart -> verify -> health.
.DESCRIPTION
...
if ($PSCmdlet.ShouldProcess($TargetRoot, "Deploy $Processor $StagedCommit")) {
    ...
    # dated backup of the current prod files before we overwrite them
    if ($BackupRoot) {
