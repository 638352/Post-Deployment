...
function Send-VesDatadogEvent {
    ...
    try {
        ...
        # Events API v1 endpoint on the GovCloud site; key sent as a header so it
        # never appears in URLs (proxy logs, IIS logs, PS transcripts).
        $uri  = "https://api.$Site/api/v1/events"
        # POST and discard the response body -- only success/failure matters here.
        Invoke-RestMethod -Uri $uri -Method Post -Body $body -ContentType 'application/json' `
            -Headers @{ 'DD-API-KEY' = $ApiKey } | Out-Null
    } catch {
...
function New-VesLogFile {
    <#
    .SYNOPSIS Build a timestamped JSONL log path (and ensure the log dir exists).
    #>
    [CmdletBinding()]
    param(
        # Log file prefix, e.g. 'drift-heartbeat-watchdog'.
        [Parameter(Mandatory)][string]$Prefix,
        # Log directory; matches the drift runner's default.
        [string]$LogDir = 'D:\ves-verify\logs'
    )
    if (-not (Test-Path -LiteralPath $LogDir)) {
        New-Item -ItemType Directory -Path $LogDir -Force | Out-Null
    }
    # UTC stamp, same format the drift runner uses, so logs sort together.
    $stamp = (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
    Join-Path $LogDir ("{0}_{1}.jsonl" -f $Prefix, $stamp)
}

function Get-VesDatadogEnvTag {
    <#
    .SYNOPSIS Normalize an environment name into the standard 'env:<name>' tag.
    #>
    [CmdletBinding()]
    param(
        # Environment name; defaults to prod because these boxes are prod-first.
...
function Get-VesAlertType {
    <#
    .SYNOPSIS Map environment to Datadog event severity: prod=error, else warning.
    #>
    [CmdletBinding()]
    param(
        [string]$Environment = 'prod'
    )
    if ($Environment -match '^(prod|production)$') { 'error' } else { 'warning' }
}
...
Export-ModuleMember -Function `
    ...
    New-VesLogFile, Get-VesDatadogEnvTag, Get-VesAlertType
